  ...
  return (
    <>
      ...
      <div className="mt-14">
        ...
        <div
          className="grid grid-cols-2 gap-4 sm:grid-cols-2 md:grid-cols-4 p-10 bottom-3"
          data-aos="fade-left"
        >
          {data.map((item, index) => (
            <div
        ...
        >
          <div
            className="fixed inset-0 z-50 flex justify-center items-center bg-black bg-opacity-75 backdrop-blur-sm"
            onClick={closeModal}
          >
            <div
              className="relative max-w-[600px] max-h-[500px]"
              onClick={(e) => e.stopPropagation()}
            >
              <button
                className="absolute top-0 right-0 m-4 h-10 w-10 bg-opacity-55 text-white text-xl p-1 bg-slate-600 rounded-full"
                onClick={closeModal}
              >
                &times;
              </button>
              <button
                className="absolute top-1/2 transform -translate-y-1/2 left-0 m-4 text-white text-2xl"
                onClick={prevImage}
              >
                &#10094;
              </button>
              <button
                className="absolute top-1/2 transform -translate-y-1/2 right-0 m-4 text-white text-2xl"
                onClick={nextImage}
              >
                &#10095;
              </button>
